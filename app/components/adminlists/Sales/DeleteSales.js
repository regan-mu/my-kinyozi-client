...
export default function DeleteSale({token}) {
    ...
    const {setSaleActions, saleIdToDelete} = useContext(SalesContext);
    ...
    const deleteSale = (e) => {
        ...
        if (passwordRef.current.value === "") {
            ...
            const axiosConfig = {
                method: "delete",
                url: `http://127.0.0.1:5000/API/sales/delete/${saleIdToDelete}`,
            ...
            axios(axiosConfig).then(
                res => {
                    ...
                    setPending(false);
                    setTimeout(() => {setSaleActions(false)}, 2000);
                    filterOutDeleted();
                }
            ).catch(
                err => {
        ...
        <div className="bg-black bg-opacity-80 fixed w-full h-screen top-0 left-0 z-20 flex justify-center items-center">
            <div className="bg-dark-blue relative flex flex-col gap-5 w-80 h-60 p-5 rounded-xl text-white">
                ...
                <div className="w-full h-auto flex justify-between items-center">
                    ...
                    <Image onClick={() => {setSaleActions(false)}} className="w-[16px] h-[16px] cursor-pointer" src="/open-menu.svg" alt="close" width={16} height={16} />
                </div>
                <form onSubmit={deleteSale} className="flex flex-col gap-3 text-gray-300">
