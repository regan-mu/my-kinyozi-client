...
export default function RemoveStaff() {
    ...
    const {setStaffActions, idToMutate} = useContext(StaffContext);
    const {setStaff} = useContext(AdminContext);
    const passwordRef = useRef();

    // Update the staff list after deletion
    const filterOutDeleted = () => {
        setStaff(prev => {
            return prev.filter(val => {return val.id != idToMutate });
        });
    }
    ...
    const submitDelete = (e) => {
        ...
        if (passwordRef.current.value === "") {
            ...
            setPending(true); 
            axios(axiosConfig("delete", `https://my-kinyozi-server.onrender.com/API/employees/remove/${idToMutate}`, {password: passwordRef.current.value})).then(
                res => {
            ...
            <div className="bg-dark-blue relative flex flex-col gap-5 w-80 h-60 p-5 rounded-xl text-white">
                <div className="w-full h-auto flex justify-between items-center">
                    ...
                    {success && <div className="w-full h-10 text-sm absolute left-0 -top-5 rounded-md flex items-center justify-center font-semibold bg-green-200 text-green-800">{success}</div>}
                        <h3 className="font-semibold text-lg">Remove Staff</h3>
                        <Image onClick={() => {setStaffActions("")}} className="w-[16px] h-[16px] cursor-pointer" src="/open-menu.svg" alt="close" width={16} height={16} />
                </div>
                <form onSubmit={submitDelete} className="flex flex-col gap-3 text-gray-300">
                    ...
                    <button disabled={pending} type="submit" className="w-full h-10 bg-red-500 rounded-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">{pending? <>Removing...</>: <>Remove</>}</button>
                </form>
            </div>
        </div>
    )
