...
export default function UpdateStaff() {
    ...
    const {setStaffActions, setStaffMutation, idToMutate} = useContext(StaffContext);
    const {staff} = useContext(AdminContext);
    const staffSelected = staff.filter(item => item?.id === idToMutate)[0] // Staff selected from the staff list to be updated
    const [formData, setFormData] = useState({
        email: staffSelected?.email,
        role: staffSelected?.role,
        salary: staffSelected?.salary,
        phone: staffSelected?.phone
    });
    ...
    const submitChanges = (e) => {
        e.preventDefault();
        setPending(true);
        axios(axiosConfig("put", `https://my-kinyozi-server.onrender.com/API/employees/update/${idToMutate}`, formData)).then(
            res => {
        ...
        <div className="bg-black bg-opacity-80 fixed w-full h-screen top-0 left-0 z-20 flex justify-center items-center overflow-hidden">
            <div className="bg-dark-blue border-[0.1px] border-accent  flex flex-col gap-5 w-96 h-auto p-5 rounded-xl text-white relative">
                <div className="w-full h-auto flex justify-between items-center">
                    ...
                    <h3 className="font-semibold text-lg">Update Staff</h3>
                    <Image onClick={closeUpdateForm} className="w-[16px] h-[16px] cursor-pointer" src="/open-menu.svg" alt="close" width={16} height={16} />
                </div>
                <form onSubmit={submitChanges} className="flex flex-col gap-2 text-gray-300">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-semibold" htmlFor="salary">Salary</label>
                        <input value={formData.salary} onChange={handleChange} className="outline-none w-full bg-dark-blue text-sm border-[0.1px] border-gray-700 h-10 p-3 rounded-md" required type="number" id="salary" name="salary" />
                    </div>
                    <div className="flex flex-col gap-2">
                    ...
                    <button disabled={pending} type="submit" className="w-full h-10 bg-secondary rounded-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">{pending ? <>Updating...</>: <>Update Staff</>}</button>
                </form>
            </div>
        </div>
    )
